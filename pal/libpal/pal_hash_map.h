...
template <class Key, class Value, class HashFunction = palHashFunction<Key>, class KeyEqual = palHashEqual<Key>, uint32_t KeyAlignment = 4, uint32_t ValueAlignment = 4>
class palHashMap
{
public:
  /* Types and constants */
...
protected:
  ...
  palArray<int, int32_t, 4>	hash_bucket_list_head_;
   ...
   * }
   */
  palArray<int, int32_t, 4>	chain_next_;
  palArray<Key, int32_t, KeyAlignment>   key_array_;
  palArray<Value, int32_t, ValueAlignment>	value_array_;
...
public:
  ...
  const palArray<Value>& GetValueArrayReference() const {
    return value_array_;
  }
  ...
  const palArray<Key>& GetKeyArrayReference() const {
    return key_array_;
  }
